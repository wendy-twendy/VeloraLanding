    ...
    <nav className={cn(
      ...
      <div className="container mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-14 sm:h-16">
          ...
          <div className="hidden md:flex items-center space-x-4 lg:space-x-8">
            {navLinks.map((link) => (
              <a 
                key={link.href}
                href={link.href} 
                className="text-sm lg:text-base text-light hover:text-primary-cyan transition-colors duration-300 font-medium"
              >
                {link.label}
              </a>
            ))}
          ...
          <button 
            type="button" 
            className="md:hidden flex items-center justify-center text-light hover:text-primary-cyan p-1.5"
            onClick={toggleMenu}
      ...
      {/* Mobile Navigation */}
      <div className={cn(
        "md:hidden bg-dark/95 border-b border-primary-cyan/20 transition-all duration-300 overflow-hidden",
        ...
        <div className="px-2 pt-2 pb-3 space-y-1">
          {navLinks.map((link) => (
            <a 
              key={link.href}
              href={link.href} 
              className="block px-3 py-2.5 text-light/90 hover:text-primary-cyan transition-colors duration-300 border-l-2 border-transparent hover:border-primary-cyan/30 rounded"
          ...
          <div className="border-t border-primary-cyan/10 mt-2 pt-2">
            <button 
              className="flex items-center justify-between w-full px-3 py-2.5 text-light/90 hover:text-primary-cyan transition-colors duration-300"
              onClick={() => {
                toggleLanguage();
                setIsMenuOpen(false);
              }}
